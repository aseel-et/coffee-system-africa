...
router.put('/', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    if (err.message.includes('no such column')) {
      try {
        db.prepare('ALTER TABLE settings ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP').run();
...
router.get('/shifts/current', authenticateToken, (req, res) => {
  try {
...
router.post('/shifts/open', authenticateToken, (req, res) => {
  try {
    const { opening_cash } = req.body;
    const existingShift = db.prepare("SELECT id FROM shifts WHERE cashier_id = ? AND status = 'open'").get(req.user.id);
    if (existingShift) return res.status(400).json({ success: false, message: 'يوجد وردية مفتوحة بالفعل' });
    
    const result = db.prepare('INSERT INTO shifts (cashier_id, opening_cash) VALUES (?, ?)').run(req.user.id, parseFloat(opening_cash) || 0);
...
router.post('/shifts/:id/close', authenticateToken, (req, res) => {
  try {
    const shiftId = parseInt(req.params.id);
    const { closing_cash, notes } = req.body;
    const shift = db.prepare('SELECT * FROM shifts WHERE id = ? AND cashier_id = ?').get(shiftId, req.user.id);
    if (!shift) return res.status(404).json({ success: false, message: 'الوردية غير موجودة' });
    
    // Calculate shift totals
    const shiftSales = db.prepare(`
