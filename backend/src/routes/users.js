...
router.post('/', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const newUser = db.prepare('SELECT id, username, full_name, role, is_active, created_at FROM users WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const updated = db.prepare('SELECT id, username, full_name, role, is_active, updated_at FROM users WHERE id = ?').get(userId);
