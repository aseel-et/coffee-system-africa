...
router.get('/', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const countResult = db.prepare(query.replace('SELECT e.*, ec.name as category_name, ec.color as category_color, u.full_name as created_by_name', 'SELECT COUNT(*) as total')).get(...params);
...
router.get('/categories', authenticateToken, requireAdmin, (req, res) => {
  try {
...
router.post('/categories', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const result = db.prepare('INSERT INTO expense_categories (name, color) VALUES (?, ?)').run(name.trim(), color || '#6B7280');
...
router.put('/categories/:id', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const { name, color } = req.body;
    const cat = db.prepare('SELECT * FROM expense_categories WHERE id = ?').get(id);
...
router.post('/', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const expense = db.prepare('SELECT e.*, ec.name as category_name FROM expenses e LEFT JOIN expense_categories ec ON ec.id = e.expense_category_id WHERE e.id = ?').get(result.lastInsertRowid);
...
router.put('/:id', authenticateToken, requireAdmin, (req, res) => {
  try {
    ...
    const updated = db.prepare('SELECT e.*, ec.name as category_name FROM expenses e LEFT JOIN expense_categories ec ON ec.id = e.expense_category_id WHERE e.id = ?').get(expId);
